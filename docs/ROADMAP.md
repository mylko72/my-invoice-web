...
**핵심 사용자 여정:**
```
담당자 → /invoice/[id] 또는 /invoice/[id]?token=[accessToken]
          ↓ page.tsx  : getInvoice(id) Notion API 조회 (60초 캐시)
          ↓ 서버 컴포넌트: validateAccessToken() + isInvoiceValid() 검증
            (accessToken 필드 있음: token 파라미터 필수 & 일치 확인)
            (accessToken 필드 없음: token 파라미터 선택적 — ID 기반 접근 제어)
          ↓ 견적서 UI 렌더링
          ↓ PDF 다운로드 또는 인쇄
```
...
- [x] **Proxy 접근 제어** (`src/proxy.ts`)
  - `/invoice/:path*` 경로 기본 라우팅 (실제 검증은 page.tsx에서 수행)
  - MVP 단계에서는 ID 기반 접근 제어 (Notion 페이지 ID 자체가 32자리 hex로 추측 불가능)
  - 향후 accessToken 필드 추가 시 강화된 검증으로 확장 가능
...
- [x] `/invoice/[유효ID]` 접근 시 견적서 페이지 렌더링 (token 파라미터 없어도 가능)
- [x] `/invoice/[유효ID]?token=[any]` 접근 시에도 견적서 페이지 렌더링
...
#### 2-1. 공통 레이아웃 컴포넌트 정비 ✅ 완료

- [x] **`NavBar` 정비** (`src/components/layout/NavBar.tsx`)
  - 견적서 뷰 페이지에서는 `print:hidden` 클래스 적용
  - 서비스 로고 / 이름 표시
- [x] **`Footer` 정비** (`src/components/layout/Footer.tsx`)
  - 서비스명, 저작권 텍스트
  - `print:hidden` 적용
...
- [x] **`InvoiceStatusBadge`** (`src/components/invoice/InvoiceStatusBadge.tsx`)
  - `draft` → 회색, `sent` → 파란색, `accepted` → 초록색
  - Tailwind CSS 색상 변수 활용

#### 2-4. 로딩 스켈레톤 ✅ 완료

- [x] **`src/app/invoice/[id]/loading.tsx`** 구현
  - A4 비율 카드 스켈레톤 (`animate-pulse`)
  - 헤더, 테이블, 합계 섹션 형태 반영

### 완료 기준 ✅ 완료

- [x] `src/components/invoice/` 하위 6개 컴포넌트 파일 생성
- [x] 각 컴포넌트가 Props 인터페이스 기반으로 TypeScript 오류 없이 렌더링
- [x] `npm run lint` 통과
- [x] 스켈레톤 UI가 실제 견적서 레이아웃과 동일한 형태로 표시

---

## Phase 3: 핵심 기능 개발 — 견적서 뷰 UI ✅ 완료 (2026-03-25)
...
파일: `src/app/invoice/[id]/page.tsx` ✅ **완료 (2026-03-25)**
...
- [x] `InvoiceHeader` 컴포넌트 통합
- [x] `InvoiceClientInfo` 컴포넌트 통합
- [x] `InvoiceItemsTable` 컴포넌트 통합
- [x] `InvoiceSummary` 컴포넌트 통합
- [x] `InvoiceNote` 컴포넌트 통합 (note가 빈 문자열이면 렌더링 생략)
- [x] `InvoiceActions` 컴포넌트 통합 (클라이언트 컴포넌트)
- [x] 페이지 최외곽 레이아웃: A4 비율 카드 (`max-w-4xl mx-auto`)
- [x] 임시 인라인 UI 코드 제거
...
- [x] `InvoiceItemsTable` 모바일 가로 스크롤 처리 (`overflow-x-auto`)
- [x] `InvoiceHeader` 모바일 세로 스택 레이아웃
- [x] 버튼 영역 모바일 전체 너비 (`w-full sm:w-auto`)

#### 3-3. 인쇄 CSS 적용 ✅ 완료

- [x] `InvoiceActions` — `print:hidden`으로 액션 버튼 숨김
- [x] `NavBar`, `Footer` — `print:hidden`
- [x] 인쇄 시 페이지 여백 제거: `print:p-0 print:m-0`
- [x] 견적서 카드 인쇄 시 그림자 제거: `print:shadow-none`
- [x] `@page` CSS 규칙 (`globals.css`에 추가):
  ```css
  @media print {
    @page {
      margin: 15mm;
...
#### 3-4. 접근 거부 페이지 UX 개선 ✅ 완료

파일: `src/app/denied/page.tsx`

- [x] `reason` 없이 직접 `/denied` 접근 시 기본 메시지 표시 (구현됨)
- [x] 오류 코드별 아이콘 차별화 (ShieldX/Clock/FileX/AlertTriangle)
- [x] 재시도 버튼 추가 (`SERVER_ERROR` 케이스에만 표시)
- [x] 모바일 레이아웃 (이미 반응형으로 구현됨)

### 완료 기준

- [x] `/invoice/[id]?token=[valid]` 접속 시 견적서 전체 UI 렌더링 (코드 구현 완료)
- [x] 품목이 0개인 경우 "견적 항목이 없습니다" 빈 상태 표시 (InvoiceItemsTable line 75-83)
- [x] 반응형 레이아웃 (모바일/태블릿/데스크톱) — Tailwind breakpoint 사용
- [x] 인쇄 CSS 적용 (@media print, A4 페이지 설정)
- [x] `npm run lint` 통과 ✅
- [x] `npm run build` 성공 ✅ (2026-03-25)

**참고:** 실제 Notion API 연동 및 브라우저 렌더링 테스트는 .env.local 설정 후 `npm run dev`로 진행
...
```
Day 1  (03-23) [완료] ████████████████ Phase 1: 프로젝트 초기 설정(골격 구축)
Day 2  (03-24) [완료] ████████████████ Phase 2: 공통 모듈/컴포넌트 개발
              [완료] ████████████████ Phase 3: 핵심 기능 개발 (Phase 2와 병렬)
Day 3  (03-25) [완료] ████████████████ Phase 3: 견적서 뷰 UI 완성
